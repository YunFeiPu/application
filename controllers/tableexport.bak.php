...
class tableexport extends CI_Controller {
    ...
    function test(){
        ...
        if(sizeof($query)<=13){
        $PHPExcel = $reader->load(getcwd()."/peisong.xls");
        }
        else{
        ...
        try {
        	...
        	$objWriter ->save(getcwd().'/download/'.$data['otime'].'/'.$_ic.'-'.$data['otime'].'.xls');
        } catch (Exception $e) {
        	//$objWriter ->save(getcwd().'/download/'.$data['otime'].'/'.$_ic.'-'.$data['otime'].'-'.'.xls');
        }
    ...
    function index(){
        ...
        */
        ...
        header('Content-Disposition: attachment;filename="'.$data['otime'].'.xls"');  
